...
$\Delta t$ is $(\texttt{as\_of} - \texttt{observed\_at})$ in whole days. If
$\Delta t < 0$ (observation after document `as_of`, clock skew), clamp
$\Delta t$ to $0$ before applying the table — do not treat negative age as
stale and do not invent a second branch that would disagree with this clamp.
