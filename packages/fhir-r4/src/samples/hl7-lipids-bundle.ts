...
/**
 * HL7's lipid report example, verbatim: a collection Bundle carrying a
 * DiagnosticReport and the four Observations it points at.
 *
 * Fetched from: https://hl7.org/fhir/R4/diagnosticreport-example-lipids.json
 *
 * A foreign R4 payload with everything real foreign payloads have — its own ids, its
 * own `fullUrl` base, a narrative naming the subject in free text, intra-bundle
 * references between five resources, and references to an Organization that is not
 * in the bundle at all. Rewriting the first four without touching a value, a code or
 * a unit, and leaving the fifth exactly as it arrived, is the whole job of
 * `normaliseBundle`.
 *
 * Not registered for the HL7 validator, and the reason is recorded rather than
 * remembered: `Identifier.system` here is `http://acme.com/lab/reports`, which the
 * validator rejects as an example URL. Normalisation does not repair it — rewriting
 * an identifier system to please a validator is the silent corruption this package
 * refuses to commit.
 */
export const HL7_LIPIDS_BUNDLE: Bundle<FhirResource> = {
  resourceType: 'Bundle',
  type: 'collection',
  entry: [
    {
