...
export async function getFhirDailyActivityFromOuraData(
  ...
  if (request.type === 'daily_activity') {
    return mapOuraDailyActivityToFHIR(inferredData as SupportedSchemaTypes['daily_activity'], 'unknown');
  }
  return [];
}

export async function getFhirSleepFromOuraData(request: RequestParams, bearerToken: string): Promise<Observation[]> {
  const data = await requestOuraData(request, bearerToken);
  const inferredData = inferOuraResponse(data);
...
export async function getFhirHeartRateFromOuraData(
  ...
  if (request.type === 'heartrate') {
    return mapOuraHeartRateToFHIR(inferredData as SupportedSchemaTypes['heartrate']);
  }
  return [];
}

export async function getFhirSpo2FromOuraData(request: RequestParams, bearerToken: string): Promise<Observation[]> {
  const data = await requestOuraData(request, bearerToken);
  const inferredData = inferOuraResponse(data);
...
export async function getFhirPersonalFromOuraData(request: RequestParams, bearerToken: string): Promise<Bundle> {
  const data = await requestOuraData(request, bearerToken);
  const inferredData = inferOuraResponse(data);
